...
def MQTT_publish(weight, unit, mitdatetime, hasImpedance, miimpedance):
    ...
    new_json = {
        ...
        "weight_unit": str(unit),
        "bmi": "{:.2f}".format(lib.getBMI()),
        "basal_metabolism": "{:.2f}".format(lib.getBMR()),
        "visceral_fat": "{:.2f}".format(lib.getVisceralFat()),
    ...
    try:
        ...
        # Check if the last weight entry is different from the new weight
        if not data or data[-1]['weight'] != new_json['weight']:
